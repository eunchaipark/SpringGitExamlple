...
// import org.jsoup.Jsoup;  // 이 줄 주석처리
// import org.jsoup.safety.Safelist;  // 이 줄 주석처리
...
public class BoardController {
	...
	private String cleanHtml(String input) {
		...
		// 기본적인 보안 처리: 위험한 HTML 태그들 제거
		return input.replaceAll("<script[^>]*>.*?</script>", "")  // script 태그 제거
					.replaceAll("<style[^>]*>.*?</style>", "")    // style 태그 제거
					.replaceAll("<iframe[^>]*>.*?</iframe>", "")  // iframe 태그 제거
					.replaceAll("javascript:", "")                 // javascript: 제거
					.replaceAll("on\\w+\\s*=", "");              // onclick 등 이벤트 핸들러 제거
	}
	...
	public String writeArticle(Board board, BindingResult results, RedirectAttributes redirectAttrs) {
		...
		try{
			...
			// Jsoup 대신 간단한 HTML 정리 사용
			board.setTitle(cleanHtml(board.getTitle()));
			board.setContent(cleanHtml(board.getContent()));
